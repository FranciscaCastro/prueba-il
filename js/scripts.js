(function ($) {
  ...
  $(window).on('load', function () {
    ...
    if ($('.cv-titles').length > 0) {
      ...
      $(
        '.cv-title, .cv-summary, .cv-discover, .cv-slide-pagination i , .cv-slide-fraction span'
      ).css({
        color: cbColor,
      });
    ...
    if ($('.c-parallax-image').length > 0) {
      ...
      var cParallaxImage =
        document.getElementsByClassName('has-parallax-image');
    ...
    $('.colova-project-button').on('click', function () {
      var targetProject = $(this).parents('.cf-title-slide');
      var clickedTitle = $('.cf-title-slide').find('.colova-project-title');

      window.clickedTitle = clickedTitle;
      if ($('.cf-titles').length > 0) {
        cfTitles.autoplay.stop();
      }

      if ($('.cv-titles').length > 0) {
        cvTitles.autoplay.stop();
      }
    });
